...
description: >
  Investigates any topic -- technical, product/market, or domain -- producing
  structured, pipeline-aware research artifacts optimized for the agentic
  PRD → Architecture → Implementation → Execution pipeline. Searches web,
  Slack, Confluence, Jira, Gmail, Gong, Snowflake, and Figma with adaptive
  depth calibration. Use when: "agentic research", "pipeline research",
  "research for PRD", "research for architecture", or research specifically
  intended to feed the agentic development pipeline (agentic-prd →
  agentic-architecture → agentic-implementation → agentic-execution).
  Not for general-purpose research outside the pipeline -- use /research for that.
---
...
1. **Search first, conclude second.** Always search and read sources BEFORE forming conclusions. Never write a finding then look for citations to justify it. Up to 57% of LLM citations are post-rationalized — defeat this by reversing the order.
...
3. **Lead with the answer.** Follow the Pyramid Principle: assertion first, grouped supporting arguments second, evidence third. Conclusions first, details on demand.
...
5. **Disclose progressively.** Structure output as: summary (5-10 lines) → key findings with recommendations → detailed evidence and analysis → source appendix.
6. **Structure for the pipeline.** Map output sections directly to what downstream agentic skills (PRD, Architecture, Implementation, Execution) need.
...
These are the non-negotiable behaviors. Violating any of these invalidates the research:

- **Search THEN conclude.** Never reverse this order. If you catch yourself writing a claim without a source, stop and search first.
- **Every claim needs a citation.** No unsourced assertions in the final artifact. If you cannot find a source, flag the claim as inference and state your confidence level.
- **Adapt the plan when evidence demands it.** If early findings invalidate your research plan, pivot. Do not follow a stale plan to completion.
- **Present conflicts, don't hide them.** When sources disagree, investigate why and present both sides. Never silently pick a winner.
